...
1. Reuse existing GES legality conditions.
2. Reuse existing CPDAG operators.
3. Reuse existing scoring functions.
4. Preserve the existing public API.
5. Implement FGES as an optional execution mode rather than a separate estimator.
6. Parallelize only candidate scoring while keeping graph modifications sequential.
7. Support degree-bounded search through `max_neighbors`.
